...
async def debug_model_paths(current_user: dict = Depends(get_current_user)):
    """TEMPORARY: Debug model paths in production"""
    import subprocess
    from pathlib import Path
    ...
    debug_info = {
        "environment_vars": {
            ...
            "K12_MODELS_DIR": os.getenv('K12_MODELS_DIR', 'NOT SET'),
        },
    ...
    for path in production_paths:
        ...
        if exists and path_obj.is_dir():
            try:
                pkl_files = [f.name for f in path_obj.glob("*.pkl")][:10]
                debug_info["paths_checked"][path]["pkl_files"] = pkl_files
            except Exception as e:
                debug_info["paths_checked"][path]["error"] = str(e)
    
    # Check specific problem file
    problem_file = "/opt/render/project/results/models/best_binary_model.pkl"
    debug_info["problem_file"] = {
        "path": problem_file,
        "exists": Path(problem_file).exists()
    }
    ...
    try:
        ...
        debug_info["file_search"]["command"] = "find /opt/render/project -name best_binary_model.pkl"
        debug_info["file_search"]["found_at"] = result.stdout.strip().split('\n') if result.stdout.strip() else []
    except Exception as e:
        debug_info["file_search"]["error"] = str(e)
    ...
    for dir_path in key_dirs:
        if Path(dir_path).exists():
            try:
                ...
                debug_info["directory_contents"][dir_path] = contents[:20]  # Limit to first 20 items
            except Exception as e:
                debug_info["directory_contents"][dir_path] = {"error": str(e)}
