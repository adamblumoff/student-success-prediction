    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_1.1fr]">
      <form action={formAction} className="card flex flex-col gap-6">
        <div>
          ...
          <h2 className="mt-4 text-2xl font-semibold text-ink-50 text-balance">
            Upload your latest CSV
          </h2>
      ...
      <div className="bg-panel rounded-3xl p-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-ink-50 text-balance">Latest summary</h3>
        ...
        {state.status !== 'success' && (
          <div className="mt-4 space-y-4 text-sm text-ink-400">
            ...
            <div className="rounded-2xl border border-ink-700/60 bg-ink-950/60 p-4">
              <p className="text-xs uppercase text-ink-500">Recent uploads</p>
          ...
          <div className="mt-6 space-y-4">
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="rounded-2xl bg-rose-900/40 p-3">
                <p className="text-xs uppercase text-rose-200">High</p>
                <p className="mt-1 text-xl font-semibold text-rose-100 tabular-nums">
                  {state.result.summary.high}
                </p>
              </div>
              <div className="rounded-2xl bg-amber-900/40 p-3">
                <p className="text-xs uppercase text-amber-200">Moderate</p>
                <p className="mt-1 text-xl font-semibold text-amber-100 tabular-nums">
                  {state.result.summary.medium}
                </p>
              </div>
              <div className="rounded-2xl bg-sage-900/40 p-3">
                <p className="text-xs uppercase text-sage-200">Low</p>
                <p className="mt-1 text-xl font-semibold text-sage-100 tabular-nums">
                  {state.result.summary.low}
                </p>
              </div>
            </div>
            <div>
              <h4 className="text-sm font-semibold text-ink-200 text-balance">Top predictions</h4>
                ...
                {state.result.predictions.slice(0, 8).map((prediction) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-xs text-ink-400">
                      GPA{' '}
                      <span className="tabular-nums">{prediction.current_gpa ?? '-'}</span> ·
                      Attendance{' '}
            ...
            <div className="rounded-2xl border border-ink-700/60 bg-ink-950/50 p-4 text-xs text-ink-400">
              Previewed{' '}
              <span className="tabular-nums">
                {Math.min(state.result.predictions.length, 8)}
              </span>{' '}
              of{' '}
              <span className="tabular-nums">
